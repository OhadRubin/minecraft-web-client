...
An experimental Android-friendly version of the controller is available in `android_controller.py`. It uses the [Kivy](https://kivy.org) framework and maintains a persistent WebSocket connection in a background thread.

### Running on Android

1. Install the `kivy` and `websockets` Python packages.
2. Run `python android_controller.py` on your device, or package it with [Buildozer](https://github.com/kivy/buildozer) to create an APK.

The Android app exposes the same joystick and buttons as the pygame version but uses touch events optimized for mobile. A label at the top left shows the current connection status. Press the back button (or `ESC`) to quit and `R` to reconnect.

You can override the WebSocket address by setting the `MINECRAFT_WS_URI` environment variable before starting the app. A helper `requirements_kivy.txt` file lists the minimal dependencies:
...
You can also pass options to override the WebSocket address or flip the layout:

```bash
python android_controller.py --uri ws://192.168.1.100:8081 --left-handed
```

- `--uri` overrides the WebSocket address (equivalent to setting `MINECRAFT_WS_URI`).
- `--left-handed` mirrors the joystick and buttons for left-handed use (or set `LEFT_HANDED=1`).
